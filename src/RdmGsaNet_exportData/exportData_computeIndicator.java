...
public class exportData_computeIndicator extends exportData_main {
	...
	public static void runMultiLineIndicators ( boolean run ) throws IOException {
		exportData_csv.computeMultiLineIndicator(run, 5,  2500, typeMultiLineIndicator.degreeDistribution, pathToStore , folder , 10 );
		exportData_csv.computeMultiLineIndicator(run, 5,  2500, typeMultiLineIndicator.normalDegreeDistribution, pathToStore , folder , 10 );
	}
...
//		exportData_csv.computeSimpleIndicator( run , 5, 2000, typeIndicator.averageShortpath, pathToStore , folder );		
		exportData_csv.computeSimpleIndicator( run , 5, 2500, typeSimpleIndicator.density, pathToStore , folder );
		exportData_csv.computeSimpleIndicator( run , 5, 2500, typeSimpleIndicator.diameter, pathToStore , folder );
		exportData_csv.computeSimpleIndicator( run , 5, 2500, typeSimpleIndicator.gammaIndex, pathToStore , folder );
